...
- **Browser Performance Issues**: Both X and YouTube scrapers use non-headless browsers to bypass anti-bot detection. If experiencing high resource usage on small servers, the browsers are optimized with `--disable-images`, `--disable-audio-output`, and other performance flags. Check system memory/CPU usage and ensure Xvfb virtual display is running properly. See `CLAUDE.md` for detailed browser optimization documentation.
- **YouTube/X Scraper Timeouts**: If seeing "Timeout 30000ms exceeded" errors, this typically indicates anti-bot detection blocking the browser. Ensure the scraper is using `headless: false` configuration and Xvfb virtual display is properly configured. The bot automatically handles this with performance-optimized browser arguments.
