...
describe('URL Regex Pattern Tests', () => {
  ...
  beforeEach(() => {
    ...
    tweetUrlRegex = /https?:\/\/(?:[\w-]+\.)*(?:x\.com|twitter\.com|vxtwitter\.com|fxtwitter\.com|nitter\.[^\/]+)\/(?:(?:i\/web\/)?status(?:es)?|[^\/]+\/status(?:es)?)\/(\d+)(?:\?|#|\s|$)/g;
  });
  ...
  describe('Regex Performance and Edge Cases', () => {
    it('should handle very long text with many URLs efficiently', () => {
      const longText = Array(1000).fill(0).map((_, i) => 
        `Text ${i} https://www.youtube.com/watch?v=video${i.toString().padStart(7, '0')} more text`
      ).join(' ');
